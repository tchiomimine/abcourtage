...
(function($, Drupal) {
  ...
  var App = {
    ...
    init: function() {
      ...
      // Scroll to news
      // console.log(get_param('page'));
      if (get_param('page')) {
        $.smoothScroll({
          offset: -180,
          scrollTarget: '#newsListing'
        });
      }
      ...
      sr.reveal('.pCta .wrapper', animationLight);
      sr.reveal('.blockLarge', animationLight);
      sr.reveal('.newsBlock', animationLight);
      sr.reveal('.home .newsletter', animationLight);
      sr.reveal('.pJob', animationLight);

      App['home'].init();

      // Lexique
      if (document.getElementsByClassName("page-lexiquelisting")[0]) {
        App['lexique'].init();
      }

      // actualité
      if (document.getElementsByClassName("page-newslisting")[0]) {
        App['actu'].init();
      }

      // Map regions
      if (document.getElementById("map")) {
        App['map_regions'].init();
      }

      // Agence
      if (document.getElementsByClassName("tpl-agency")[0]) {
        App['agence'].init();
      }

      // Newsletter
      if (document.getElementById("abc-newsletter-inscription-form")) {
        App['newsletter'].init();
      }
    ...
    home: {
      init: function() {
        ...
        sr.reveal('.block', animationBlock, 80);
        ...
        sr.reveal('.step', animationStep, 80);

        var indexImg = Math.floor(Math.random() * 4) + 2;
        $('.home .banner').css("background-image", "url(/sites/all/themes/custom/adt/dist/images/home_" + indexImg + ".jpg)");
      }
    },
    ...
    optins: {
      init: function() {
        ...
        if ($('#edit-optin-newsletter').prop('checked') == false && $('#edit-optin-partenaires').prop('checked') == false ) {
          $('#optins-form #edit-submit').attr("disabled", true);
        }

        $('#optins-form #edit-optin-newsletter').on("click", function(){
          if ($('#edit-optin-newsletter').prop('checked') == false && $('#edit-optin-partenaires').prop('checked') == false ) {
            $('#optins-form #edit-submit').attr("disabled", true);
          }
          else {
            $('#optins-form #edit-submit').attr("disabled", false);
          }
        });

        $('#optins-form #edit-optin-partenaires').on("click", function(){
          if ($('#edit-optin-newsletter').prop('checked') == false && $('#edit-optin-partenaires').prop('checked') == false ) {
            $('#optins-form #edit-submit').attr("disabled", true);
          }
          else {
    ...
    agence: {
      ...
      init: function() {
        ...
        function setPercent(elem) {
          /*var skills = $(".skill-percentWrapper");
          $.each(skills, function(index, item){
            var percent = $(item).css('width');
            $(item).css('width', 0);
            $(item).animate({
              width: percent
            }, 3000);
        ...
        var animationPercent = {
          ...
          scaler: 1.2,
          delay: 500,
          beforeReveal: setPercent
        };
        sr.reveal('.skill-percentWrapper', animationPercent, 80);
    ...
    lexique: {
      init: function() {
        ...
        var indexOffset = 0;
        var stateResearch = false;

        $(".lexique-header").stick_in_parent({
          offset_top: 60
        });

        // $('a').smoothScroll({offset: -70});

        var allDt = $('.lexique-listing').find('dt');
        var allDef = $('.lexique-listing').find('.def');
        var allDl = $('.lexique-listing').find('dl');
        ...
        function initArrOffset() {

          $.each(allDl, function(i) {
            var offsetTop = $(allDl[i]).position().top;
            var height = $(allDl[i]).height()
            var id = $(allDl[i]).attr('id');
            mapIndexLetter[id.toLowerCase()] = i;
            offSetLetter.push({
              'offset': offsetTop,
              'height': height,
              'letter': id
        ...
        function initCurrentLetter() {

          $.each(offSetLetter, function(i) {

            if ($(window).scrollTop() >= offSetLetter[i].offset) {

              indexOffset = i;
              $('.lexique-currentletter').text(offSetLetter[i].letter);
        ...
        $('.lexique-navigation li span').click(function() {

          $('#listLexique input').val('');
          $.each(valueArr, function(i) {
            $(valueArr[i].elt).css('display', 'block');
          });
          ...
          $.each(offSetLetter, function(i) {
            if (offSetLetter[i].letter == anchor) {
              indexOffset = i;
            }
          });
          $('.lexique-currentletter').text(anchor);

        });

        $('.listLexique .deleteSearch').click(function() {
          ...
          indexOffset = 0;
          $('.lexique-currentletter').text(offSetLetter[0].letter);
        ...
        $(window).scroll(function(event) {
          ...
          if(stateResearch === true){
            return false;
          }

          st = $(this).scrollTop() + startOffSet;

          if (st > offSetLetter[19].offset) {
            $('.lexique-currentletter').text(offSetLetter[19].letter);
            return false;
          }

          if (offSetLetter[indexOffset] !== undefined) {

            if (st > lastScrollTop) {

              if ($(this).scrollTop() + startOffSet > offSetLetter[indexOffset].offset) {
                $('.lexique-currentletter').text(offSetLetter[indexOffset].letter);
                indexOffset++;
              }

            } else {

              if ($(this).scrollTop() - offSetLetter[indexOffset].height < offSetLetter[indexOffset].offset) {
                $('.lexique-currentletter').text(offSetLetter[indexOffset].letter);
                indexOffset--;
              }
        ...
        $.each(allDt, function(i) {
          var match = $(allDt[i]).text();
        ...
        var searchFieldPosY = $('#listLexique').position().top;
        searchFieldPosY = parseInt(searchFieldPosY)-100;
        var searchField = $('#listLexique input');
        ...
        searchField.keyup(function() {
          ...
          if (val.length >= 2) {
            var currentLetter = val.substr(0,1);
            window.location.href = "#"+currentLetter.toUpperCase();

            stateResearch = true;


            $.each(valueArr, function(i) {
              var valSearch = valueArr[i].title.substr(0, val.length);
              var valSearch2 = valueArr[i].match.substr(0, val.length);

              valSearch = valSearch.toLowerCase();
              valSearch2 = valSearch2.toLowerCase();



              currentLetter = currentLetter.toLowerCase();

              indexOffset = mapIndexLetter[currentLetter];
              $('.lexique-currentletter').text(offSetLetter[indexOffset].letter);

              if (valSearch != val && valSearch2 != val) {
                $(valueArr[i].elt).css('display', 'none');
            ...
            if (nbResult === 1) {
              $('dl .def').css('padding-bottom', '200px');
            } else {
              $('dl .def').css('padding-bottom', '0px');
            }
    ...
    newsletter: {
      init: function() {
        ...
        $('#abc-newsletter-inscription-form #edit-submit').click(function(event) {
          var mail = $('#abc-newsletter-inscription-form #edit-email').val();
          var re = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}/igm;
          if (re.test(mail) !== true) {
            var error = $('#abc-newsletter-inscription-form .field-email .error');
            if (error.length == 0) {
              $('#abc-newsletter-inscription-form .field-email').append('<div class="error"</div>');
            }
            $('#abc-newsletter-inscription-form .field-email .error').html('Veuillez indiquer une adresse email valide');
            event.preventDefault();
          }
        });
      }
    ...
    actu: {
      init: function() {
        ...
        var currentIndex = $('.pagination').find('.current').index();
        $.each(item, function(i) {
          if (i > 1 && i < item.length - 3) {
            if (i < currentIndex - 3 ||
              i > currentIndex + 3
            ) {
    ...
    map_regions: {
      ...
      init: function() {
        ...
        if (get_param('region')) {
          $.smoothScroll({
            offset: -90,
            scrollTarget: '#regionsListing'
          });
        }

        var map = document.querySelector('#map');
        var paths = document.querySelectorAll('.map-main a');
        var links = document.querySelectorAll('.map-aside a');

        paths.forEach(function(region) {
          active_region(region);
        });

        links.forEach(function(region) {
          active_region(region);
        });

        function active_region(region) {
          region.addEventListener('mouseover', function(e) {
            map.querySelectorAll('.is-hover').forEach(function(item) {
              item.classList.remove('is-hover')
            });
            var id = this.id.replace('list-', '');
            document.querySelector('#list-' + id).classList.add('is-hover');
            document.querySelector('#' + id).classList.add('is-hover');
          })
        }
      }
    } // ! map_regions
...
(function ($) {
  ...
  var burger = debounce(function() {

    if($('.ctnBurger').css('display') == 'none'){
      mobile = false;
    }else{
      mobile = true;
    }
  ...
  $(window).resize(burger);

  var positionMenu = 0;
  var st = null;
  var lastScrollTop = 0;
  var initialValue = $('.w-menu').outerHeight(true);

  $('#btnMenu').click(function(){
    $(this).toggleClass('cross');
    $('.header').toggleClass('open');
    var offsetTop = $(window).scrollTop();
    var headerOffsetBottom = $('.w-menu').outerHeight(true) + offsetTop;

    var stateSticky = $('header').hasClass('sticky');

    if (stateSticky) {
      var headerOffsetBottom = offsetTop + $('.credit-mention').innerHeight()-2;
    }

    $('.navBar').css({
      position : 'absolute',
      top : headerOffsetBottom+'px',
      padding:'0 0 10px 0',
      left: 'auto'
    });

    positionMenu = headerOffsetBottom;
  });
          ...
          st = $(this).scrollTop();
            ...
            if (st <   lastScrollTop){
              ...
              if(st < positionMenu){
                ...
                var posY = $('.w-menu').outerHeight(true);

                if($('header').hasClass('sticky') === false){
                  var posY = initialValue;
              ...
              if(position === 'fixed'){
                var posY = $('.w-menu').outerHeight(true) + st;

                // if($('header').hasClass('sticky')){
                //   var mentionHeigth = $('.credit-mention').outerHeight();
                //   posY -= mentionHeigth;
                // }
...
(function($) {
  ...
  var validateForm = function(input,target,callback){
    //input : verification que l'on souhaite effectuer ()
    // isCheck
    // isOptional
    // isEmpty
    // isCity (code postal)
    ...
    // target : container du ou des input(s) que l'on souhaite valider
    ...
    switch (input) {
      ...
      case 'isRadio':
        var fields = target.querySelectorAll('input[type="radio"]');
        elt = target.querySelector('.field');
        ...
        if (checked == false ) {
          validate = false;
          warning = "Veuillez sélectionner une option";
        }
        else {
          validate = true
          warning = '';
      ...
      case 'isEmpty':
        var fields = target.querySelectorAll('input[type="text"], textarea');
        validate = true;
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i];
          if(ignore === false && elt.value === ''){
            validate = false;
            warning = "Veuillez compléter l'information demandée";
            return false;
          }
        });
        break;
      ...
      case 'isEmptyFirstName':
        ...
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i];
          if(ignore === false && elt.value === '' || re.test(elt.value) === false){
            validate = false;
            warning = "Veuillez indiquer votre prénom";
            return false;
          }
        });
        break;
      ...
      case 'isEmptyName':
        ...
        $.each(fields,function(i){
          ...
          if(ignore === false && elt.value === '' || re.test(elt.value) === false){
            validate = false;
            warning = "Veuillez indiquer votre nom";
            return false;
          }
        });
        break;
      case 'isEmptyMessage':
        ...
        $.each(fields,function(i){
          ...
          if(ignore === false && elt.value === ''){
            validate = false;
            warning = "Veuillez écrire votre message";
            return false;
          }
        });
        break;
      case 'isCP':
        var fields = target.querySelectorAll('input#code_postal');
        validate = true;
        var re = /^[0-9]{5}$/;
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i];
          if(ignore === false && elt.value.length != 5 && re.test(elt.value) !== true){
            validate = false;
            warning = 'Vous devez renseigner un code postal valide';
            return false;
          }
        });
        break;
      case 'isCity':
        ...
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          var city = $(this).parents('.isCity').find('#ville').val();
          elt = fields[i];

          if(!city){
            validate = false;
            warning = 'Vous devez renseigner la ville dans laquelle se situe votre projet';
            return false;
          }
        ...
        break;
      case 'isCity2':
        var fields = target.querySelectorAll('input[type="text"]');
        validate = true;
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i];
      ...
      case 'isDate':
        ...
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i];
          var splitDate = elt.value.split('/');
          var currentTime = new Date();

          var years = currentTime.getFullYear()-15;

          if(splitDate.length != 3 ||
              parseInt(splitDate[0]) > 31 ||
              parseInt(splitDate[1]) > 12 ||
              parseInt(splitDate[2]) > years ||
              parseInt(splitDate[2]) < 1900 ||
              splitDate[0] == "00" ||
              splitDate[1] == "00"
          ){
            validate = false;
            warning = 'Veuillez indiquer une date au format : jj/mm/aaaa';
            return false;
          }
        });
        break;
      case 'isPhoneNumber':
        ...
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i];
          var firstDigits = elt.value.substring(0, 2);
          var prefix = elt.value.substring(0, 1);

          if(ignore === false && elt.value.length != 10 || firstDigits == "00" || prefix !== "0"){
            validate = false;
            warning = 'Veuillez indiquez votre numéro de téléphone';
            return false;
          }
        });
        break;
      case 'isEuro':
        ...
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i];
          if(re.test(elt.value) !== true && ignore === false || elt.value == "0"){
            validate = false;
            warning = 'Veuillez indiquez votre montant';
            return false;
          }
        });
        break;
      ...
      case 'isEuroRevenu':
        ...
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i]
          if(re.test(elt.value) !== true && ignore === false){
            validate = false;
            warning = "Veuillez indiquer votre revenu, si vous ne percevez pas de revenu, indiquez 0";
            return false;
          }
        });
        break;
      ...
      case 'isEuroApport':
        ...
        $.each(fields,function(i){
          ...
          if(re.test(elt.value) !== true && ignore === false){
            validate = false;
            warning = "Veuillez indiquer votre montant, si vous n'avez pas d'apport personnel indiquez 0";
            return false;
          }
        });
        break;
      case 'isEuroPret':
        ...
        $.each(fields,function(i){
          ...
          if(re.test(elt.value) !== true && ignore === false){
            validate = false;
            warning = "Veuillez indiquer votre montant, si vous n'avez pas de prêt immobilier indiquez 0";
            return false;
          }
        });
        break;

      case 'isEuroEstimationBien':
        var re = /^[0-9, ]+$/;
        var fields = target.querySelectorAll('input[type="text"]');
        validate = true;
        $.each(fields,function(i){
          ...
          if(re.test(elt.value) !== true && ignore === false){
            validate = false;
            warning = "Veuillez indiquer votre montant, si vous n'avez pas de bien immobilier, indiquez 0";
            return false;
          }
        });
        break;

      case 'isEuroRachatImmo':
        var re = /^[0-9, ]+$/;
        var fields = target.querySelectorAll('input[type="text"]');
        validate = true;
        $.each(fields,function(i){
          ...
          if(re.test(elt.value) !== true && ignore === false){
            validate = false;
            warning = "Veuillez indiquer votre montant, si vous n'avez pas de crédit immobilier à racheter, indiquez 0";
            return false;
          }
        });
        break;

      case 'isEuroRachatConsommation':
        var re = /^[0-9, ]+$/;
        var fields = target.querySelectorAll('input[type="text"]');
        validate = true;
        $.each(fields,function(i){
          ...
          if(re.test(elt.value) !== true && ignore === false){
            validate = false;
            warning = "Veuillez indiquer votre montant, si vous n'avez pas de crédit à la consommation à racheter, indiquez 0";
            return false;
          }
        });
        break;
      ...
      case 'isEmail':
        var re = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}/igm;
        var fields = target.querySelectorAll('input[type="text"], input[type="email"]');
        validate = true;
        $.each(fields,function(i){
          var ignore = $(this).hasClass('ignore');
          elt = fields[i];
          if(re.test(elt.value) !== true && ignore === false){
            validate = false;
            warning = 'Veuillez indiquer une adresse email valide';
            return false;
          }
        });
        break;
    ...
    if(warning !== null){
      ...
      var existingWarning = target.querySelectorAll('.warning');
      ...
      if(existingWarning.length === 0){
        ...
        var elmtWarning = $('<span/>');
        elmtWarning.addClass('warning');
        setTimeout(function(){
          elmtWarning.animate({
            opacity : 1,
          },'slow')
        ...
        elmtWarning.text(warning);

        if(input == 'isCity'){
          var target = $(elt).parents();
          elmtWarning.insertAfter($('.field-ville #ville'));
        }else{
          elmtWarning.insertAfter(elt);
        }
      }
    }

    if(validate === true){
      var warning = target.getElementsByClassName('warning');
      if(warning.length > 0){
        warning[0].remove();
      }
    }
    ...
    if (typeof callback === "function") {
      callback(validate,warning);
    }
    return validate
  };

  var controlPhone = function(elt){
    var val = elt.val();
    var saveVal ="";
    var re = /^[0-9]+$/;
    var character = val.slice(-1);

    for (var i = 0; i < val.length; i++) {
      if(re.test(val.charAt(i)) === false){
        var newVal = val.substr(0, i)+val.substring(i+1);
        elt.val(newVal);
        val = newVal;
      }
    }
  ...
  window.controlPhone = controlPhone;
...
(function($) {
  ...
  var menuForm = {
    init : function(items,ListNav,ListIndex){

        // items : elements que l'on souhaite placer dans le menu
        // ListNav : element ul dans lequel on crée les elements du menu
        // indexNav : element ul dans lequel on crée le numero de l'etape

        var stepsFormList = ListNav;
        var indexNav = ListIndex;

        $.each(items,function(i){

          var li = $('<li/>');
          var arrow = $('<div class="arrow"><span></span><span></span></div>');
          var num = $('<span class="numero"><span>'+(i+1)+'<span/></span>');
          var num2 = $('<li><span class="numero-index">'+(i+1)+'<span/></li>');
          var content = $('<span/>').text(items[i].innerText);

          li.appendTo(stepsFormList);
          num.appendTo(li);
          num2.appendTo(indexNav);
    ...
    selectItem : function (active,ListNav,ListIndex,allSection){

      // active : index du li que l'on souhaite selectionner
      // ListNav : element ul dans lequel se trouve les elements du menu
      // indexNav : element ul dans lequel se trouve le numero de l'etape
      //allSection : ensemble des sections du formulaire

      var items = ListNav.children('li');
      var itemsIndex = ListIndex.children('li');

      $.each(allSection,function(i){
        if(active != i){
          if($(allSection[i]).hasClass('active') === true){
            $(allSection[i]).removeClass('active');
            $(allSection[i]).addClass('hide');
          }
        }else{
          $(allSection[i]).addClass('active');
          $(allSection[i]).removeClass('hide');
        }
      });
      ...
      $.each(items,function(i){
        if(active == i){
          ...
          $(itemsIndex[i]).addClass('selected');
          $(itemsIndex[i]).removeClass('off');

        }else if(active < i){

          $(items[i]).addClass('off');
          $(items[i]).removeClass('selected');

          $(itemsIndex[i]).removeClass('selected');
          $(itemsIndex[i]).addClass('off');
...
(function($) {
  ...
  var stepField = function(stepFieldCurrent,stepFieldNext){

      var stepFormField = stepFieldCurrent;

      var containerStep = stepFormField.parents('section');

      var posY = containerStep[0].querySelectorAll('.stepsForm-field')[0].offsetTop;
      if(window.innerWidth >= 900) posY-=60;

      var current = stepFormField.attr('id');
      var last = stepFormField.hasClass('is-last')
      current = parseInt(current);


      var allSteps = containerStep[0].querySelectorAll('.stepsForm-field');
      if(current >= 0){
        for(var i = 0; i <= current; i++){
          posY += allSteps[i].offsetHeight;
        }
      }

      /// VERIF
      var validate = false;
      var elt = stepFormField[0];
      var verif = elt.classList[0];
      validateForm(verif,elt,function(boolean){
        validate = boolean;
      });
      //

      if(last === false && validate === true){

        var ignore = $(stepFieldNext).hasClass('ignore');

        if(ignore === true){
          var nextOne = $(stepFieldNext).next();
          while (nextOne.hasClass('ignore')) {
              nextOne = $(nextOne).next()
          }
         stepFieldNext = nextOne;
        }
        ...
        if(window.innerWidth >= 900){
          if($(stepFieldNext).hasClass('is-last') === false){
           $('.stepsForm-nav').animate({
        ...
        if(ignore === false){
          stepFieldNext.addClass('show');
          var eltFocus = stepFieldNext.find('input[type=text], textarea');
          if(eltFocus[0] !== undefined){
            eltFocus[0].focus();
          }
        }else{
          for(var i = 2;i <100; i++){
            if(allSteps[parseInt(current+i)] !== undefined){
              var ignore = $(allSteps[parseInt(current+i)]).hasClass('ignore');
              if(ignore === false){
                allSteps[parseInt(current+i)].classList.add('show');
                var eltFocus = $(allSteps[parseInt(current+i)]).find('input[type=text], textarea');
                var isLast = $(allSteps[parseInt(current+i)]).hasClass('is-last');
                if(eltFocus[0] !== "undefined" && isLast === false){
                  eltFocus[0].focus();
                }
                return;
              }
...
(function ($, Drupal) {
  ...
  var formAcquisition = {
    init: function(){
      // MASK
        ...
        $('#steps-acquisition .date').mask('00/00/0000',{placeholder: ".. /.. /...."},options);
        ...
        //$('#steps-acquisition .phone').mask('0000000000');
        $('#steps-acquisition .euro').mask('000 000 000 000 000',{reverse: true});

      var btnValidate = null;
      var currentField = null;

      // $('#steps-acquisition input[type="text"], #steps-acquisition select').focus(function(){
      //   btnValidate = $(this).parents('.stepsForm-field').find('button');
      //   currentField = $(this).parents('.stepsForm-field');
      // });
      //
      ...
      // NAVIGATION FORM
      var stepAcquisition = $('#steps-acquisition .stepsForm-main');
      var allSection = $('#steps-acquisition .stepsForm-main').children('section');
      var step = 0;
      var fields = [];
      var menuItems = [];

      $.each(allSection,function(i){
        var fieldsBySection = allSection[i].querySelectorAll('.stepsForm-field');
        menuItems.push(allSection[i].getElementsByTagName('h2')[0]);
        $.each(fieldsBySection,function(i){
          $(this).attr('id',i);
          fields.push(fieldsBySection[i]);
        })
      })
      $.each(fields,function(i){
        if(i == 0){
      ...
      //MENU
      var stepsFormList = $('#steps-acquisition #stepsForm-list');
      var indexNav = $('#steps-acquisition #indexNav');

      menuForm.init(menuItems,stepsFormList,indexNav);
      menuForm.selectItem(step,stepsFormList,indexNav,allSection);

      //FIELD-STEP

      $('#steps-acquisition section:not(.info-perso) div:not(.container-switch) input[type="radio"]:not()').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        if($(this).parents("#edit-credits-presence").length != 1){
          stepField(stepFormField,stepFormField.next());
        }
      });

      $('#steps-acquisition #edit-co-empruteur-presence .option').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        stepField(stepFormField,stepFormField.next());
      });

      $('#steps-acquisition section button:not(.next)').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        // btnValidate = btnValidate.parents('.stepsForm-field').next('.stepsForm-field').find('button');
      ...
      $('#steps-acquisition #edit-co-empruteur-presence input').on('click',function(e){

        val = $(this).val();

        if(val == 0){
          $('#steps-acquisition .co_emprunteur_revenus').each(function(){
            $(this).css('display','none');
          ...
          $('#steps-acquisition #co_emprunteur_revenus').css('display','none');
          $('#steps-acquisition #co_emprunteur_revenus').find('input').addClass('ignore');
          $('#steps-acquisition #co_emprunteur_revenus').find('input').val('');
          ...
          $('#steps-acquisition #co_emprunteur_revenus').css('display','inherit');
          $('#steps-acquisition #co_emprunteur_revenus').find('input').removeClass('ignore');
        }
      });

      // PRESENCE CREDIT
      $('#steps-acquisition #edit-credits-presence input').on('click',function(e){
        var val = $(this).val();
        var stepFormField = $(this).parents('.stepsForm-field');
        if(val == "0"){
          $('#steps-acquisition .field-credits-montant').css('display','none');
          $('#steps-acquisition .field-credits-montant').addClass('ignore');
          $('#steps-acquisition .field-credits-montant').find('input').addClass('ignore');
          var nextField = $('#steps-acquisition .field-credits-montant').parent()
          nextField.removeClass('show');
          stepField(stepFormField,nextField.next());
        }else{
          $('#steps-acquisition .field-credits-montant').css('display','inherit');
          $('#steps-acquisition .field-credits-montant').removeClass('ignore');
          $('#steps-acquisition .field-credits-montant').find('input').removeClass('ignore');
          stepField(stepFormField,stepFormField.next());
        }
      ...
      $('#steps-acquisition .next').on('click',function(e){

        var section = $(this).parents('section');

        $('#stepsForm-list li').each(function(i){
          $(this).on('click',function(){
            goToStep(section,false,allSection,i);
          });
        })

        $('#indexNav li').each(function(i){
          $(this).on('click',function(){
            goToStep(section,false,allSection,i);
          });
        })
        ...
        step++;
        goToStep(section,true,allSection,step);

      });

      function goToStep(section,validate,sectionsAll,target){

        var verifFields = section[0].getElementsByClassName('stepsForm-field');
        var next = true;
        ...
        if(validate === true){
          $.each(verifFields,function(i){
            if(verifFields[i] !== undefined){
              var verif = verifFields[i].classList[0];
              if(verif !== 'stepsForm-field'){
                validateForm(verif,verifFields[i],function(boolean,warning){
                  if(boolean === false){
                    next = false;
                  }
                });
        ...
        if(next === true){
          posY = 0;
          $('.stepsForm-nav').css('margin-top',posY+"px");
          $('#html, body').scrollTop(posY);


          if(target !== 0){
            sectionsAll[target-1].classList.add('hide','complete');
          }

          if($(sectionsAll[target]).hasClass('hide')){
            $(sectionsAll[target]).removeClass('hide');
          }

          var current = $(this).closest('.stepsForm-field').attr('id');
          current = parseInt(current);
          var firstStep = sectionsAll[target].getElementsByTagName('div')[0]
          firstStep.classList.add('show');
          var eltFocus = $(firstStep).find('input[type="text"]')[0];
          if(eltFocus !== undefined){
            eltFocus.focus();
          }
          step = target;
          menuForm.selectItem(target,stepsFormList,indexNav,allSection);
          if(validate === true){
            tracking(target);
          }
        }
      ...
      function tracking(currentIndex){
        /// SUR L'ETAPE 2 ENVOYER currentIndex 1
        dataLayer.push({
          'event':'VirtualPageview',
          'virtualPageURL': 'acquisition/' + currentIndex
        });
      }

      //SUBMIT

      $('#steps-acquisition #submitForm').click(function(event){
        ...
        $.each(verifFields,function(i){
          var verif = verifFields[i].classList[0];
          if(verif !== 'stepsForm-field'){
            var test = validateForm(verif,verifFields[i],function(boolean){
              if(boolean === false){
                event.preventDefault();
                return boolean;
              }
      ...
      $('#steps-acquisition #ville').on('keyup', function () {
        var query = this.value;
        if (query) {
          $.getJSON("/ajax/get/zip/" + query, function(result) {
            $('#listVille').empty();
            var listVille = $('#listVille');
            $.each(result, function(i) {
              ...
              var city = $('<span/>')
                  ...
                  .text(' - ' + result[i].name)
                  .appendTo(li);
              li[0].addEventListener('click', selectCity);
            });
          });
        }
        else {
      ...
      // SEARCH CODE POSTAL VILLE
      /*$('#steps-acquisition #ville').vicopo(function (cities) {
      ...
      function selectCity(e){
        var warning = $('#steps-acquisition .field-ville').find('.warning');
        if(warning[0] !== undefined){
          $(warning[0]).remove();
        }
        var zip = $(this).children("#zipCode");
        var city = $(this).children("#city");

        var selectedZipCode = zip[0].innerText;
        var selectedCity = city[0].innerText;

        $("#code_postal").val(selectedZipCode);
        $("#ville").val(selectedZipCode+selectedCity);
        $( ".resultCity" ).remove();
      }
      ...
      $("#steps-acquisition #edit-nature .option").click(function(){
        var nature = $(this).attr('for');
        if(nature == 'edit-nature-neuf'){
          rate = 0.035
        }else if (nature == 'edit-nature-ancien') {
          rate = 0.085
        }
      })

      $('#steps-acquisition #validateApport').click(function(){
        var apport = $('#edit-apport-personnel').val();
        var amount = $('#edit-prix-achat').val();
        ...
        var charges = parseInt(rate*amount);
        var total = rate*amount + parseInt(amount);
        var pourcentageFrais = (charges*100 )/ total;
        var pourcentageApport = (apport*100 )/ total;

        var valueArr = [apport,amount,charges,total];

        $('#apport').text(Math.round(valueArr[0])+' €');
        $('#prices').text(Math.round(valueArr[1])+' €');
        $('#charges').text(Math.round(valueArr[2])+' €');
        $('#total').text(Math.round(valueArr[3])+' €');

        var pourcentageFraisRound = (Math.round(pourcentageFrais * 10) / 10);
        $('.chart .pourcentage span').text(pourcentageFraisRound+"%");

        if(rate === 0.035){
          $('.chart .pourcentage').addClass('neuf');
          $('.chart .pourcentage').removeClass('ancien');
        }else{
          $('.chart .pourcentage').addClass('ancien');
          $('.chart .pourcentage').removeClass('neuf');
        }

        $('.chart .pourcentage').animate({
          opacity : 1,
        },'slow');

        var dataset = [
          { label: 'Frais', count: pourcentageFrais },
          { label: 'Total', count: 100-pourcentageFrais },
        ];
        // ec660b
        // d89d75
        if($('#chartPie').find('svg').length > 0){
          $('#chartPie').find('svg').remove();
          chartpie(dataset);
        }else{
          chartpie(dataset);
        }
  ...
  $(document).ready(function() {
    ...
    if (document.getElementsByClassName("page-leadacquisition-form")[0]) {
      formAcquisition.init();
    }
...
(function ($) {
  ...
  var affilies = function(){

    // $('#webform-client-form-191 #edit-submitted-telephone').mask('0000000000');

    $('#edit-submitted-telephone').keyup(function(){
      controlPhone($(this));
    })

    $('.webform-submit').click(function(e){

      var form = $('#webform-client-form-191');
      var verifFields = form.find('.isEmail, .isEmpty, .isPhoneNumber, .isCity2, .isEmptyFirstName, .isEmptyName, .isEmptyMessage');
      $.each(verifFields,function(i){
        var verif = verifFields[i].classList[0];
        var test = validateForm(verif,verifFields[i],function(boolean){
          if(boolean === false){
    ...
    $('#webform-client-form-191 #edit-submitted-ville').vicopo(function (cities) {
      ...
      var listVille = $('#listVille');
      if(cities.length > 0){
        listVille.addClass("active");
      }else {
        listVille.removeClass("active");
      }
    ...
    function selectCity(e){
      ...
      $("#edit-submitted-ville").val(selectedZipCode+selectedCity);
      $('#listVille li').remove();
      listVille.removeClass("active");
  ...
  $(document).ready(function() {
    ...
    if (document.getElementById("webform-client-form-191")) {
      affilies();
    }
...
(function ($, Drupal) {
  ...
  var formRachat = {
    init: function(){
      ...
      $('#steps-rachat .date').mask('00/00/0000',{placeholder: "  /  /    "},options);
      //$('#steps-rachat .phone').mask('0000000000');
      $('#steps-rachat .euro').mask('000 000 000 000 000',{reverse: true});
      ...
      // NAVIGATION FORM
      var stepAcquisition = $('#steps-rachat .stepsForm-main');
      var allSection = $('#steps-rachat .stepsForm-main').children('section');
      var step = 0;
      var fields = [];
      var menuItems = [];
      ...
      var stepsFormList = $('#steps-rachat #stepsForm-list');
      var indexNav = $('#steps-rachat #indexNav');
      ...
      $('#steps-rachat section:not(.info-perso) input[type="radio"]').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');

        if($(this).parents("#edit-tresorerie").length != 1 &&
            $(this).parents("#edit-co-empruteur-presence").length != 1 &&
            $(this).parents("#edit-logement").length != 1
        ){
          stepField(stepFormField,stepFormField.next());
        }
      });
      ...
      $('#steps-rachat section button:not(.next)').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        stepField(stepFormField,stepFormField.next());
      });
      ...
      $('#steps-rachat #edit-co-empruteur-presence input').on('click',function(e){
        val = $(this).val();
        var stepFormField = $(this).parents('.stepsForm-field');

        if(val == 0){
          $('#steps-rachat .co_emprunteur').each(function(){
            $(this).css('display','none');
            $(this).addClass('ignore');
            $(this).find('input').addClass('ignore');
          });

          $('#steps-rachat .field-co-emprunteur-situation-professionnelle').parent().removeClass('show');
          $('#steps-rachat .field-co-emprunteur-situation-professionnelle').parent().addClass('ignore');
          ...
          $('#steps-rachat .field-co-emprunteur-situation-professionnelle').parent().addClass('show');
          $('#steps-rachat .field-co-emprunteur-situation-professionnelle').parent().removeClass('ignore');
          stepField(stepFormField,stepFormField.next());
      ...
      // TYPE DE PRET
      $('#steps-rachat #edit-type-pret input').on('click',function(e){
        val = $(this).val();
        var stepFormField = $(this).parents('.stepsForm-field');
        ...
        if(val == 'conso'){
          ...
          // Hide Immo
          $('#steps-rachat .field-montant-credits-immobilier').parent().removeClass('show');
          $('#steps-rachat .field-montant-credits-immobilier').parent().addClass('ignore');
          $('#steps-rachat .field-montant-credits-immobilier').css('display','none');
          $('#steps-rachat .field-montant-credits-immobilier').addClass('ignore');
          $('#steps-rachat .field-montant-credits-immobilier').find('input').addClass('ignore');
          $('#steps-rachat .field-montant-credits-immobilier').find('input').val('');

          // Show Conso
          $('#steps-rachat .field-montant-credits-consommation').parent().addClass('show');
          $('#steps-rachat .field-montant-credits-consommation').parent().removeClass('ignore');
          $('#steps-rachat .field-montant-credits-consommation').css('display','inherit');
          $('#steps-rachat .field-montant-credits-consommation').removeClass('ignore');
          $('#steps-rachat .field-montant-credits-consommation').find('input').removeClass('ignore');
          $('#steps-rachat .field-montant-credits-consommation').find('input').val('');

          // Hide Estimation bien
          $('#steps-rachat .field-estimation-bien').parent().removeClass('show');
          $('#steps-rachat .field-estimation-bien').parent().addClass('ignore');
          $('#steps-rachat .field-estimation-bien').css('display','none');
          $('#steps-rachat .field-estimation-bien').addClass('ignore');
          $('#steps-rachat .field-estimation-bien').find('input').addClass('ignore');
          $('#steps-rachat .field-estimation-bien').find('input').val('');
          ...
          // Show Immo
          $('#steps-rachat .field-montant-credits-immobilier').parent().addClass('show');
          $('#steps-rachat .field-montant-credits-immobilier').parent().removeClass('ignore');
          $('#steps-rachat .field-montant-credits-immobilier').css('display','inherit');
          $('#steps-rachat .field-montant-credits-immobilier').removeClass('ignore');
          $('#steps-rachat .field-montant-credits-immobilier').find('input').removeClass('ignore');
          $('#steps-rachat .field-montant-credits-immobilier').find('input').val('');

          // Hide Conso
          $('#steps-rachat .field-montant-credits-consommation').parent().removeClass('show');
          $('#steps-rachat .field-montant-credits-consommation').parent().addClass('ignore');
          $('#steps-rachat .field-montant-credits-consommation').css('display','none');
          $('#steps-rachat .field-montant-credits-consommation').addClass('ignore');
          $('#steps-rachat .field-montant-credits-consommation').find('input').addClass('ignore');
          $('#steps-rachat .field-montant-credits-consommation').find('input').val('');


          stepField(stepFormField,stepFormField.next());

        } else if (val == 'conso-immo'){
      ...
      // TYPE LOGEMENT
      $('#steps-rachat #edit-logement input').on('click',function(e){
        val = $(this).val();
        var stepFormField = $(this).parents('.stepsForm-field');

        // Show/Hide Estimation bien
        var val_type_pret = $('#steps-rachat #edit-type-pret input[name=type_pret]:checked').val();
        if(val_type_pret == 'conso'){
          // Hide Estimation bien
          $('#steps-rachat .field-estimation-bien').parent().removeClass('show');
          $('#steps-rachat .field-estimation-bien').parent().addClass('ignore');
          $('#steps-rachat .field-estimation-bien').css('display','none');
          ...
          // Show Estimation bien
          $('#steps-rachat .field-estimation-bien').parent().addClass('show');
          $('#steps-rachat .field-estimation-bien').parent().removeClass('ignore');
          $('#steps-rachat .field-estimation-bien').css('display','inherit');
          $('#steps-rachat .field-estimation-bien').removeClass('ignore');
          $('#steps-rachat .field-estimation-bien').find('input').removeClass('ignore');
          $('#steps-rachat .field-estimation-bien').find('input').val('');
        }
        ...
        if(val == 'locataire'){
          ...
          // Show loyer
          $('#steps-rachat .field-montant-loyer').parent().addClass('show');
          $('#steps-rachat .field-montant-loyer').parent().removeClass('ignore');
          $('#steps-rachat .field-montant-loyer').css('display','inherit');
          $('#steps-rachat .field-montant-loyer').removeClass('ignore');
          $('#steps-rachat .field-montant-loyer').find('input').removeClass('ignore');
          $('#steps-rachat .field-montant-loyer').find('input').val('');
          ...
          // Hide loyer
          $('#steps-rachat .field-montant-loyer').parent().removeClass('show');
          $('#steps-rachat .field-montant-loyer').parent().addClass('ignore');
          $('#steps-rachat .field-montant-loyer').css('display','none');
          $('#steps-rachat .field-montant-loyer').addClass('ignore');
          $('#steps-rachat .field-montant-loyer').find('input').addClass('ignore');
          $('#steps-rachat .field-montant-loyer').find('input').val('');
      ...
      $('#steps-rachat #edit-tresorerie input').on('click',function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        ...
        if(val == 'non'){
          ...
          $('#steps-rachat .field-tresorerie-montant').parent().removeClass('show');
          $('#steps-rachat .field-tresorerie-montant').parent().addClass('ignore');
          $('#steps-rachat .field-tresorerie-montant').css('display','none');
          $('#steps-rachat .field-tresorerie-montant').addClass('ignore');
          $('#steps-rachat .field-tresorerie-montant').find('input').addClass('ignore');
          $('#steps-rachat .field-tresorerie-montant').find('input').val('');

          stepField(stepFormField,stepFormField.next());

        }else{
          $('#steps-rachat .field-tresorerie-montant').parent().addClass('show');
          $('#steps-rachat .field-tresorerie-montant').parent().removeClass('ignore');
          $('#steps-rachat .field-tresorerie-montant').css('display','inherit');
          $('#steps-rachat .field-tresorerie-montant').removeClass('ignore');
          $('#steps-rachat .field-tresorerie-montant').find('input').removeClass('ignore');
      ...
      $('#steps-rachat .next').on('click',function(e){
        var section = $(this).parents('section');
      ...
      function tracking(currentIndex){
        ...
        dataLayer.push({
          'event':'VirtualPageview',
          'virtualPageURL': 'rachat/' + currentIndex
        });
      }

      //SUBMIT

      $('#steps-rachat #submitForm').click(function(event){
      ...
      // SEARCH CODE POSTAL VILLE
      $('#steps-rachat #ville').on('keyup', function () {
        var query = this.value;
        if (query) {
          $.getJSON("/ajax/get/zip/" + query, function(result) {
            $('#listVille').empty();
      ...
      /*$('#steps-rachat #ville').vicopo(function (cities) {

        if ($('#ville').val() === '') {
          $("#edit-code-postal").val('');
          return;
        }

        $( ".resultCity" ).remove();
        var result = cities.slice(0,4);
        var listVille = $('#listVille');
        $.each(result,function(i){
          var li = $('<li/>')
              .addClass('resultCity')
      ...
      function selectCity(e){
        var warning = $('#steps-rachat .field-ville').find('.warning');
        if(warning[0] !== undefined){
          $(warning[0]).remove();
        }
        var zip = $(this).children("#zipCode");
  ...
  $(document).ready(function() {
    ...
    if (document.getElementsByClassName("page-leadrachat-form")[0]) {
      formRachat.init();
    }
...
(function ($, Drupal) {
  ...
  var formRenegociation = {
    init: function(){
      ...
      // MASK
        ...
        $('#steps-renegociation .date').mask('00/00/0000',{placeholder: "  /  /    "},options);
        //$('#steps-renegociation .phone').mask('0000000000');
        $('#steps-renegociation .euro').mask('000 000 000 000 000',{reverse: true});
      ...
      // NAVIGATION FORM
      var stepAcquisition = $('#steps-renegociation .stepsForm-main');
      var allSection = $('#steps-renegociation .stepsForm-main').children('section');
      var step = 0;
      var fields = [];
      var menuItems = [];
      ...
      //MENU
      var stepsFormList = $('#steps-renegociation #stepsForm-list');
      var indexNav = $('#steps-renegociation #indexNav');

      menuForm.init(menuItems,stepsFormList,indexNav);
      menuForm.selectItem(step,stepsFormList,indexNav,allSection);

      // RENEGOCIER UN OU ENSEMBLE DE PRET ETAGE 1 QUESTION 1
      var statePret;

      $('#steps-renegociation #edit-projet input').on('click',function(e){
        statePret = $(this).val();

        if(statePret != 'conso'){
          $('#steps-renegociation .field-restant-consommation').css('display','none');
          $('#steps-renegociation .field-restant-consommation').find('input').val('');
          $('#steps-renegociation .field-restant-consommation').find('input').addClass('ignore');

          $('#steps-renegociation .field-restant-consommation').parent().addClass('ignore');

          $('#steps-renegociation .field-restant-consommation').parent().removeClass('show');
        }else{
          $('#steps-renegociation .field-restant-consommation').css('display','inherit');
          $('#steps-renegociation .field-restant-consommation').find('input').removeClass('ignore');
          $('#steps-renegociation .field-restant-consommation').parent().removeClass('ignore');

          if($('#steps-renegociation .field-restant-immobilier').parent().hasClass('show') === true ){
            $('#steps-renegociation .field-restant-consommation').parent().addClass('show');
          }
      ...
      $('#steps-renegociation section:not(.info-perso) input[type="radio"]').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        stepField(stepFormField,stepFormField.next());
      });

      $('#steps-renegociation #edit-co-empruteur-presence .option').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        stepField(stepFormField,stepFormField.next());
      });

      $('#steps-renegociation section button:not(.next)').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        stepField(stepFormField,stepFormField.next());
      });
      ...
      $('#steps-renegociation #edit-co-empruteur-presence input').on('click',function(e){

        val = $(this).val();

        if(val == 0){
          $('#steps-renegociation .co_emprunteur_revenus').each(function(){
            $(this).css('display','none');
            $(this).addClass('ignore');
            $(this).find('input').addClass('ignore');

          });
          $('#steps-renegociation #co_emprunteur_revenus').css('display','none');
          $('#steps-renegociation #co_emprunteur_revenus').find('input').addClass('ignore');
          $('#steps-renegociation #co_emprunteur_revenus').find('input').val('');
        }else{
          $('#steps-renegociation .co_emprunteur_revenus').each(function(){
            $(this).css('display','inherit');
            $(this).removeClass('ignore');
          ...
          $('#steps-renegociation #co_emprunteur_revenus').css('display','inherit');
          $('#steps-renegociation #co_emprunteur_revenus').find('input').removeClass('ignore');
        }
      });

      //STEP

      $('#steps-renegociation .next').on('click',function(e){
      ...
      function tracking(currentIndex){
        ...
        dataLayer.push({
          'event':'VirtualPageview',
          'virtualPageURL': 'renegociation/' + currentIndex
        });
      }

      //SUBMIT

      $('#steps-renegociation #submitForm').click(function(event){
        var section = $(this).parents('section');
        var verifFields = section[0].getElementsByClassName('stepsForm-field');
        var submit = true;
        $.each(verifFields,function(i){
      ...
      $('#steps-renegociation #ville').on('keyup', function () {
        var query = this.value;
        if (query) {
          $.getJSON("/ajax/get/zip/" + query, function(result) {
            $('#listVille').empty();
      ...
      /*$('#steps-renegociation #ville').vicopo(function (cities) {
      ...
      function selectCity(e){
        var warning = $('#steps-renegociation .field-ville').find('.warning');
        if(warning[0] !== undefined){
          $(warning[0]).remove();
        }
        var zip = $(this).children("#zipCode");
  ...
  $(document).ready(function() {
    ...
    if (document.getElementsByClassName("page-leadrenegociation-form")[0]) {
      formRenegociation.init();
    }
...
(function ($, Drupal) {
  ...
  var formRenegociation2 = {
    init: function(){
      ...
      $('#steps-renegociation--2 .date').mask('00/00/0000',{placeholder: "  /  /    "},options);
      //$('#steps-renegociation--2 .phone').mask('0000000000');
      $('#steps-renegociation--2 .euro').mask('000 000 000 000 000',{reverse: true});
      ...
      // NAVIGATION FORM
      var stepAcquisition = $('#steps-renegociation--2 .stepsForm-main');
      var allSection = $('#steps-renegociation--2 .stepsForm-main').children('section');
      var step = 0;
      var fields = [];
      var menuItems = [];
      ...
      //MENU
      var stepsFormList = $('#steps-renegociation--2 #stepsForm-list');
      var indexNav = $('#steps-renegociation--2 #indexNav');
      ...
      $('#steps-renegociation--2 #edit-projet input').on('click',function(e){
        statePret = $(this).val();

        if(statePret != 'conso'){
          $('#steps-renegociation--2 .field-restant-consommation').css('display','none');
          $('#steps-renegociation--2 .field-restant-consommation').find('input').val('');
          $('#steps-renegociation--2 .field-restant-consommation').find('input').addClass('ignore');

          $('#steps-renegociation--2 .field-restant-consommation').parent().addClass('ignore');

          $('#steps-renegociation--2 .field-restant-consommation').parent().removeClass('show');
        }else{
          $('#steps-renegociation--2 .field-restant-consommation').css('display','inherit');
          $('#steps-renegociation--2 .field-restant-consommation').find('input').removeClass('ignore');
          $('#steps-renegociation--2 .field-restant-consommation').parent().removeClass('ignore');

          if($('#steps-renegociation--2 .field-restant-immobilier').parent().hasClass('show') === true ){
            $('#steps-renegociation--2 .field-restant-consommation').parent().addClass('show');
          }
      ...
      $('#steps-renegociation--2 section:not(.info-perso) input[type="radio"]').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        stepField(stepFormField,stepFormField.next());
      });

      $('#steps-renegociation--2 #edit-co-empruteur-presence .option').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        stepField(stepFormField,stepFormField.next());
      });

      $('#steps-renegociation--2 section button:not(.next)').click(function(e){
        var stepFormField = $(this).parents('.stepsForm-field');
        stepField(stepFormField,stepFormField.next());
      });
      ...
      $('#steps-renegociation--2 #edit-co-empruteur-presence input').on('click',function(e){

        val = $(this).val();

        if(val == 0){
          $('#steps-renegociation--2 .co_emprunteur_revenus').each(function(){
            $(this).css('display','none');
            $(this).addClass('ignore');
            $(this).find('input').addClass('ignore');

          });
          $('#steps-renegociation--2 #co_emprunteur_revenus').css('display','none');
          $('#steps-renegociation--2 #co_emprunteur_revenus').find('input').addClass('ignore');
          $('#steps-renegociation--2 #co_emprunteur_revenus').find('input').val('');
        }else{
          $('#steps-renegociation--2 .co_emprunteur_revenus').each(function(){
            $(this).css('display','inherit');
            $(this).removeClass('ignore');
          ...
          $('#steps-renegociation--2 #co_emprunteur_revenus').css('display','inherit');
          $('#steps-renegociation--2 #co_emprunteur_revenus').find('input').removeClass('ignore');
        }
      });

      //STEP

      $('#steps-renegociation--2 .next').on('click',function(e){
      ...
      $('#steps-renegociation--2 #submitForm').click(function(event){
        var section = $(this).parents('section');
        var verifFields = section[0].getElementsByClassName('stepsForm-field');
        var submit = true;
        $.each(verifFields,function(i){
      ...
      $('#steps-renegociation--2 #ville').on('keyup', function () {
        var query = this.value;
        if (query) {
          $.getJSON("/ajax/get/zip/" + query, function(result) {
            $('#listVille').empty();
      ...
      function selectCity(e){
        var warning = $('#steps-renegociation--2--2 .field-ville').find('.warning');
        if(warning[0] !== undefined){
          $(warning[0]).remove();
        }
        var zip = $(this).children("#zipCode");
  ...
  $(document).ready(function() {
    ...
    if (document.getElementsByClassName("page-leadrenegociation-2-form")[0]) {
      formRenegociation2.init();
    }
...
(function ($) {
  ...
  var agenceMap = {
    init : function(){
      ...
      var ApiKey = "AIzaSyBnkmTCCJw5z55ccXsTYHBk1M76u-4uZxY";

      var city = $('.agency-city .text:nth-child(2)').text();
      var zipCode = $('.agency-city .text:nth-child(1)').text();

      var street = $('.agency-address .text').text();
      street = street+" "+zipCode;

      var request = "https://web.archive.org/web/20201013074128/https://maps.googleapis.com/maps/api/geocode/json?address="+street+"&components=city:"+city+"&key="+ApiKey;
      // var request = "https://web.archive.org/web/20201013074128/https://maps.googleapis.com/maps/api/geocode/json?address="+street+"&key="+ApiKey;
      ...
      $.get( request, function( data ) {
        ...
        var map = new google.maps.Map(document.getElementById("mapAgency"), optionsMap);
        var myLatLng = new google.maps.LatLng(coord.lat, coord.lng)

        var AbIcon = new google.maps.MarkerImage('/sites/all/themes/custom/adt/dist/images/marker.png',
      ...
      $('.overlay').click(function () {
        $(this).css('height','0');
        $('#mapAgency').toggleClass('offScroll');
      });

      $('#mapAgency').mouseleave(function () {
        $(this).toggleClass('offScroll');
...
(function ($) {
  ...
  var formContact = {
    init : function(){
      $('.webform-submit').click(function(e){
        ...
        var form = $('.webform-client-form-185');
        var verifFields = form.find('.isEmail, .isEmpty, .isEmptyFirstName, .isEmptyName, .isEmptyMessage');
        $.each(verifFields,function(i){
          var verif = verifFields[i].classList[0];
          var test = validateForm(verif,verifFields[i],function(boolean){
            if(boolean === false){
  ...
  $(document).ready(function() {
    ...
    if(document.getElementById('webform-client-form-185')){
      formContact.init();
    }
...
(function ($) {
  ...
  var formGuide = {
    init : function(){
      $('.webform-submit').click(function(e){
        ...
        var form = $('.webform-client-form-188');
        var verifFields = form.find('.isEmail, .isEmpty, .isEmptyFirstName, .isEmptyName, .isEmptyMessage');
        $.each(verifFields,function(i){
          var verif = verifFields[i].classList[0];
          var test = validateForm(verif,verifFields[i],function(boolean){
  ...
  $(document).ready(function() {
    ...
    if(document.getElementById('webform-client-form-188')){
      formGuide.init();
    }
...
(function ($, Drupal) {
  ...
  function newForm(form) {
    ...
    /**
     * Init étapes du formulaire
     */
    var allSection = $('.stepsForm-main').children('section');
    var step = 0;
    var fields = [];
    var menuItems = [];
     ...
     * Init menu
     */
    var stepsFormList = $('#stepsForm-list');
    var indexNav = $('#indexNav');
    menuForm.init(menuItems,stepsFormList,indexNav);
    menuForm.selectItem(step,stepsFormList,indexNav,allSection);

    /**
     * Gestion des liens étapes dans le menu
     */
    var section = $('.stepsForm-main section.active');
    $('#stepsForm-list li').each(function(i){
      $(this).on('click',function(){
        goToStep(section,false,allSection,i);
      });
    ...
    /**
     * Gestion des liens boutons suivants
     */
      //FIELD-STEP
    $('section button:not(.next)').click(function(e){
      var stepFormField = $(this).parents('.stepsForm-field');
      stepField(stepFormField,stepFormField.next());
    });

    $('.stepsForm-main section.step1 .next').on('click',function(e){
      var section = $(this).parents('section');
      step = 1;
      goToStep(section,false,allSection,step);
    });

    $('.stepsForm-main section.step2 .next').on('click',function(e){
      var section = $(this).parents('section');
      step = 2;
      goToStep(section,false,allSection,step);
    });
    ...
    function goToStep(section,validate,sectionsAll,target){
      ...
      if(next === true){
        ...
        if(target !== 0){
          //sectionsAll[target-1].classList.add('hide','complete');
          sectionsAll[target-1].classList.add('hide');
        }
    ...
    $('.date').mask('00/00/0000',{placeholder: ".. /.. /...."},options);
    $('.phone').keyup(function(){ controlPhone($(this));})
    $('.euro').mask('000 000 000 000 000',{reverse: true});

    /**
     * Rachat type de crédit
     */
      // TYPE DE PRET
    $('#edit-type-pret input').on('click',function(e){
      val = $(this).val();

      if (val == 'conso') {
        $('.field-restant-immobilier').parent().addClass('ignore');
        $('.field-restant-immobilier').addClass('ignore');
        $('.field-restant-immobilier').find('input').addClass('ignore');
      }
      else if (val == 'immo'){
        $('.field-restant-consommation').parent().addClass('ignore');
        $('.field-restant-consommation').addClass('ignore');
        $('.field-restant-consommation').find('input').addClass('ignore');
      }
    });

    /**
     * Affichage du champ Revenu co-emprunteur
     */
    $('#edit-co-empruteur-presence input').on('click',function(e){
      val = $(this).val();
      if(val == 0){
        $('.co_emprunteur_revenus').each(function(){
          $(this).css('display','none');
          $(this).addClass('ignore');
          $(this).find('input').addClass('ignore');
        });
        $('#co_emprunteur_revenus').css('display','none');
        $('#co_emprunteur_revenus').find('input').addClass('ignore');
        $('#co_emprunteur_revenus').find('input').val('');
        ...
        $('#co_emprunteur_revenus').css('display','inline-block');
        $('#co_emprunteur_revenus').find('input').removeClass('ignore');
      }
    });

    /**
     * Affichage du champ besoin de tresorerie
     */
    $('#edit-tresorerie-oui').on('click',function(e){
      $('.tresorerie-montant').show();
    });
    $('#edit-tresorerie-non').on('click',function(e){
      $('.tresorerie-montant').hide();
    });



    /**
     * Autocomplétion champ ville
     */
    ...
    function selectCity(e){
      var warning = $('.field-ville').find('.warning');
      if(warning[0] !== undefined){
        $(warning[0]).remove();
      }
      var zip = $(this).children("#zipCode");
    ...
    /**
     * Background sur champ sélectionné
     */
    $('input').click(function() {
      $('.field-row').removeClass('selected');
      $(this).parents('.field-row').addClass('selected');
    });
    ...
    $('#submitForm').click(function(event){
      ...
      var sections = document.querySelectorAll('.stepsForm-main section');
      var stepErrorNum = -1;

      $.each( sections, function(s) {

        var section = sections[s];
        var verifFields = section.querySelectorAll('.stepsForm-field');
        var stepError = false;
        ...
        $.each(verifFields,function(i){
          ...
          if(verif !== 'stepsForm-field'){
            ...
            validateForm(verif,verifFields[i], function(boolean){
              if(boolean === false) {
        ...
        if (stepError) {
          //Change step
          $('.errors').show();
          menuForm.selectItem(stepErrorNum,stepsFormList,indexNav,allSection);
          event.preventDefault();
        }
    ...
    $('.stepsForm-field').change(function() {
      if (complete) {
        validateEachFields();
      }
    });

    function validateEachFields() {
      var sections = document.querySelectorAll('.stepsForm-main section');
    ...
    /**
     * Tracking des étapes du formulairs
     */
    function tracking(currentIndex){
      dataLayer.push({
        'event':'VirtualPageview',
        'virtualPageURL': form + '/' + currentIndex
      });
    }
     ...
     */
    $(document).on("submit", "#rappel-form", function(event){
      $(window).off('beforeunload');
    });

    $('.helpForm .form-text').mask('00 00 00 00 00',{placeholder: "_ _  _ _  _ _  _ _  _ _"},options);
    $('.submit-tel').click(function(e) {
      e.preventDefault();
      var tel = $(this).parent().find('.ct-tel').val();
      var firstDigits = tel.substring(0, 2);
      var prefix = tel.substring(0, 1);
      if (tel.length != 14 || firstDigits == "00" || prefix !== "0"){
        alert('Veuillez renseigner un numéro de téléphone valide.');
      }
      else {
        $('input#edit-tel').val(tel);
        $('#rappel-form').submit();
      }
    });
    ...
    /**
     * Avertissement en quittant la page
     */
    // Modal par défault du navigateur
    $(window).on('beforeunload', function(){
      return "Vos données ne seront pas enregistrées";
    });
    $(document).on("submit", "#acquisition-2-form", function(event){
      $(window).off('beforeunload');
    });
    $(document).on("submit", "#acquisition-2-form", function(event){
      $(window).off('beforeunload');
    });
    $(document).on("submit", "#rachat-2-form", function(event){
      $(window).off('beforeunload');
    });
  ...
  // Quit modal - redirect to page
  $('.btn-close').click(function() {
    $( ".modal" ).removeClass("open");
    $(window).off('beforeunload');
    if (window.quitPageLink[0] == '/') {
      document.location.href = "https://web.archive.org/web/20201013074128/http://abcourtage.com" + window.quitPageLink;
    }
    else {
      document.location.href = window.quitPageLink;
    }
  });

  // Add form rappel button on mobile nav
  var rappelButton = '<div class="rappel-button"></div>';
  $('#acquisition-2-form .stepsForm-nav').append(rappelButton);
  $('#rachat-2-form .stepsForm-nav').append(rappelButton);
  $('.rappel-button').click(function() {
    $('.stepsForm-nav .helpForm').toggle();
    $('.stepsForm-list').toggle();
    $('.stepsForm-main').toggle();
  });
  ...
  $(document).ready(function() {
    ...
    if (document.getElementsByClassName("page-leadacquisition-2-form")[0]) {
      newForm('acquisition_2');
    }
    if (document.getElementsByClassName("page-leadrachat-2-form")[0]) {
      newForm('rachat_2');
    }
  });
...
/*
     FILE ARCHIVED ON 07:41:28 Oct 13, 2020 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 11:10:27 Jul 08, 2021.
...
/*
playback timings (ms):
  captures_list: 100.82
  exclusion.robots: 0.258
  exclusion.robots.policy: 0.243
  RedisCDXSource: 33.21
  esindex: 0.014
  LoadShardBlock: 40.68 (3)
  PetaboxLoader3.datanode: 1117.203 (4)
  CDXLines.iter: 22.368 (3)
  load_resource: 1184.815
  PetaboxLoader3.resolve: 96.293
*/